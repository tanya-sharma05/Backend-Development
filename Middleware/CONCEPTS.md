...
A middleware is a function that sits between an incoming HTTP request and the final route handler.
...
If any middleware does not call next() or ends the response, the chain stops.
...
app.use(async (req,res,next) => {
  throw new Error("Fail"); // Express won't catch this
});
...
🛑 When Middleware Should END the Response
...
app.get('/private', (req,res) => res.send('Private area'));
...
✔ Don't mutate req wildly (use clean names: req.user)
✔ Group routes using router-level middleware
✔ Use one centralized error handler
✔ Use async wrapper to catch errors
✔ Always put error-handling middleware at the end
✔ Avoid CPU-heavy work inside middleware
✔ Log middleware order when debugging
